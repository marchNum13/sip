...
if(!isset($_GET['dari']) || !isset($_GET['sampai'])){
    header('Location: laporan-bonus-ujroh');
    exit();
...
$pdf->SetTitle('Laporan Bonus Ujroh');
$pdf->SetHeaderData('', 0, 'Laporan Bonus Ujroh', 'Periode: ' . $dari . ' 00:00 wita - ' . $sampai . ' 23:59 wita');
...
$html = '
    <h2 style="text-align: center; font-family: Arial, sans-serif; color: #4CAF50;">Laporan Bonus Ujroh</h2>
...
$pdf->Output('laporan_bonus_ujroh.pdf', 'D');
...
function dataTable(){
    ...
    $data = $userTableClass->selectUserJoin(
        fields: "
            ...
            user.email AS email,
            SUM(report_bonuses_ujroh.reward) AS total_reward
        ",
        join: "
            JOIN report_bonuses_ujroh ON user.id_user = report_bonuses_ujroh.id_user_recruiter
        ",
        key: "
            DATE_FORMAT(CONVERT_TZ(FROM_UNIXTIME(report_bonuses_ujroh.date / 1000), '+00:00', '+08:00'), '%Y-%m-%d %H:%i:%s') >= '$dari 00:00:00' AND 
            DATE_FORMAT(CONVERT_TZ(FROM_UNIXTIME(report_bonuses_ujroh.date / 1000), '+00:00', '+08:00'), '%Y-%m-%d %H:%i:%s') <= '$sampai 23:59:59'
            GROUP BY 
                user.id_user, 
                user.username, 
                user.email
